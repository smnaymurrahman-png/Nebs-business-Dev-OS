  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard icon={CalendarClock} label="Total Meetings" value={meetings.length}                                             iconBg="bg-blue-50"    iconColor="text-blue-600" />
        <StatCard icon={CalendarCheck} label="Upcoming"       value={meetings.filter((m) => new Date(m.meetingDate) >= now).length} iconBg="bg-emerald-50" iconColor="text-emerald-600" />
        <StatCard icon={CalendarX}     label="Past"           value={meetings.filter((m) => new Date(m.meetingDate) < now).length}  iconBg="bg-gray-100"   iconColor="text-gray-500" />
        <StatCard icon={AlertCircle}   label="Urgent"         value={urgent}                                                       iconBg="bg-red-50"     iconColor="text-red-500" />
      </div>

      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search meetings..." className="pl-8 pr-3 py-2 text-[13px] border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 bg-white placeholder:text-gray-300 text-gray-700 w-60 transition-all" />
        </div>
        <button onClick={() => { setForm(emptyForm); setError(""); setModal(true); }} className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 text-white text-[13px] font-semibold rounded-lg hover:bg-blue-700 active:scale-[0.98] transition-all">
      ...
      {upcoming.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-[13px] font-semibold text-gray-700">Upcoming</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {upcoming.map((m) => (
              <div key={m.id} className={`group bg-white rounded-xl border border-gray-200 p-4 space-y-3 hover:shadow-sm transition-shadow ${PRIORITY_CARD_STYLES[m.priorityLevel] ?? ""}`}>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-[13px] font-semibold text-gray-800">{m.meetingTopic}</p>
                    <p className="text-[11px] text-gray-400 mt-0.5">By {m.user?.name}</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <StatusBadge status={m.priorityLevel} label={PRIORITY_LABELS[m.priorityLevel] ?? m.priorityLevel} />
                    <button onClick={() => del(m.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all ml-1"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
                ...
                {m.assignees.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {m.assignees.map(({ user }) => (
                      <span key={user.id} className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-[11px] font-medium">{user.name}</span>
                    ))}
                  </div>
                )}
              </div>
      ...
      {past.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-[13px] font-semibold text-gray-500">Past Meetings</h3>
            <span className="bg-gray-100 text-gray-500 text-[11px] px-2 py-0.5 rounded-full font-semibold">{past.length}</span>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {past.map((m) => (
                    <tr key={m.id} className="group border-b border-gray-100 opacity-70 hover:bg-gray-50/60 transition-colors">
                      <td className="px-5 py-3.5 text-[13px] font-medium text-gray-600">{m.meetingTopic}</td>
                      <td className="px-5 py-3.5 text-[12px] text-gray-500 whitespace-nowrap">{new Date(m.meetingDate).toLocaleString()}</td>
                      <td className="px-5 py-3.5"><StatusBadge status={m.priorityLevel} label={PRIORITY_LABELS[m.priorityLevel]} /></td>
                      <td className="px-5 py-3.5 text-[12px] text-gray-500">{m.assignees.map((a) => a.user.name).join(", ") || <span className="text-gray-300">—</span>}</td>
                      <td className="px-5 py-3.5 text-[12px] text-gray-500">{m.user?.name}</td>
                      <td className="px-5 py-3.5">
                        <button onClick={() => del(m.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 className="w-3.5 h-3.5" /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {filtered.length === 0 && (
        <div className="bg-white rounded-xl border border-gray-200">
          <div className="flex flex-col items-center justify-center py-16 text-gray-400">
            ...
            <p className="text-[13px] font-medium text-gray-500">No meetings scheduled</p>
            <p className="text-[12px] mt-1">Request a meeting to get started</p>
          </div>
        </div>
      )}
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Request Meeting" size="md">
              ...
              {users.length === 0 ? (
                ...
                users.map((u) => (
                  <label key={u.id} className="flex items-center gap-2.5 cursor-pointer py-0.5">
                    ...
                    />
                    <span className="text-[13px] text-gray-700 font-medium">{u.name}</span>
                  </label>
                ))
              )}
            </div>
        ...
        <div className="flex justify-end gap-2 mt-6">
          <button onClick={() => setModal(false)} className="px-4 py-2 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600">Cancel</button>
