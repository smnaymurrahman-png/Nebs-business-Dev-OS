    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        <div className="relative flex-1 max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search meetings..." className="w-full pl-9 pr-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <button onClick={() => { setForm(emptyForm); setError(""); setModal(true); }} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 transition-colors">
              ...
              <div key={m.id} className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button onClick={() => del(m.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 ml-1"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
                ...
                {m.assignees.length > 0 && (
                  <div className="flex flex-wrap gap-1">
                    {m.assignees.map(({ user }) => (
                      <span key={user.id} className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-xs">{user.name}</span>
                    ))}
                  </div>
                )}
              </div>
