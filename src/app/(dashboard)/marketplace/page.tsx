...
import {
  Plus, Pencil, Trash2, Search, Store, BadgeDollarSign, PiggyBank, DollarSign,
...
interface MarketplaceProfile {
  ...
  profileType: string; isPaid: boolean; totalInvestment: number; amount: number;
  assignedPerson?: string; assignedDevice?: string; assignedIp?: string; createdAt: string;
  user?: { name: string };
}

const PROFILE_TYPES = ["UI_UX", "WORDPRESS", "FULL_STACK", "DIGITAL_MARKETING", "WEBFLOW_SHOPIFY_FRAMER", "OTHER"];
const emptyForm = { platformName: "", profileName: "", emailAddress: "", password: "", phoneNumber: "", specialQuestion: "", specialAnswer: "", profileType: "UI_UX", isPaid: false, totalInvestment: "", amount: "", assignedPerson: "", assignedDevice: "", assignedIp: "" };
...
function DetailRow({ label, value }: { label: string; value?: string | number | boolean | null }) {
  ...
  return (
    <div>
      ...
      <p className="text-[13px] text-gray-700">{typeof value === "boolean" ? (value ? "Yes" : "No") : String(value)}</p>
    </div>
  );
}
...
export default function MarketplacePage() {
  ...
  const load = useCallback(() => fetch("/api/marketplace").then(r => r.json()).then(setProfiles), []);
  useEffect(() => { load(); }, [load]);
  ...
  const openView = (p: MarketplaceProfile) => { setSelected(p); setModal("view"); };
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard icon={Store}           label="Total Profiles"   value={profiles.length}            iconBg="bg-blue-50"    iconColor="text-blue-600" />
        <StatCard icon={BadgeDollarSign} label="Paid Profiles"    value={paid}                       iconBg="bg-emerald-50" iconColor="text-emerald-600" />
        <StatCard icon={PiggyBank}       label="Total Investment" value={formatCurrency(totalInvestment)} iconBg="bg-orange-50" iconColor="text-orange-600" />
        <StatCard icon={DollarSign}      label="Total Value"      value={formatCurrency(totalValue)} iconBg="bg-amber-50"   iconColor="text-amber-600" />
      </div>

      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <h2 className="text-[13px] font-semibold text-gray-700">Marketplace Profiles</h2>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search profiles…" className="pl-8 pr-3 py-2 text-[13px] border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400/30 focus:border-blue-400 bg-white placeholder:text-gray-300 text-gray-700 w-52 transition-all" />
            </div>
              ...
              {filtered.length === 0 ? (
                <tr><td colSpan={11}>
                  <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                    <div className="w-12 h-12 rounded-xl bg-gray-100 flex items-center justify-center mb-3"><Store className="w-5 h-5 text-gray-300" /></div>
                    <p className="text-[13px] font-medium text-gray-500">No profiles yet</p>
                    <p className="text-[12px] mt-1">Add your first marketplace profile to get started</p>
                  </div>
                </td></tr>
              ) : filtered.map(p => (
                <tr key={p.id} className="border-b border-gray-100 hover:bg-blue-50/30 transition-colors">
                  <td className="px-5 py-3.5 text-[13px] font-semibold text-gray-700">{p.platformName}</td>
                  <td className="px-5 py-3.5 text-[13px] text-gray-600">{p.profileName}</td>
                  <td className="px-5 py-3.5 text-[13px] text-gray-500 max-w-[160px] truncate">{p.emailAddress}</td>
                  <td className="px-5 py-3.5 text-[13px] text-gray-500">{p.phoneNumber}</td>
                  <td className="px-5 py-3.5"><StatusBadge status={p.profileType} label={PROFILE_TYPE_LABELS[p.profileType] ?? p.profileType} /></td>
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold ${p.isPaid ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100" : "bg-gray-100 text-gray-600"}`}>
                      {p.isPaid ? "Paid" : "Free"}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3.5 text-[13px] text-gray-500">{p.assignedPerson || <span className="text-gray-300">—</span>}</td>
                  <td className="px-5 py-3.5 text-[12px] text-gray-400 whitespace-nowrap">{formatDate(p.createdAt)}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1">
                      <button onClick={() => openView(p)} className="flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-semibold bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors" title="View details">
                        <Eye className="w-3.5 h-3.5" /> View
                      </button>
                      <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors" title="Edit"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={() => del(p.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500 transition-colors" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                ...
                <DetailRow label="Account" value={selected.isPaid ? "Paid" : "Free"} />
                <DetailRow label="Total Investment" value={formatCurrency(selected.totalInvestment)} />
              ...
              {(selected.assignedPerson || selected.assignedDevice || selected.assignedIp) && (
                <>
                  <div className="h-px bg-gray-100" />
      ...
      {/* ── Add / Edit modal ─────────────────────────────────── */}
      <Modal isOpen={modal === "add" || modal === "edit"} onClose={() => setModal(null)} title={modal === "add" ? "Add Marketplace Profile" : "Edit Profile"} size="xl">
        {error && <p className="mb-3 text-sm text-red-600 bg-red-50 p-3 rounded-lg font-medium">{error}</p>}
        <div className="grid grid-cols-2 gap-4">
          {([["Platform Name", "platformName", "Upwork"], ["Profile Name", "profileName", "Display name"], ["Email Address", "emailAddress", "email@example.com"], ["Password", "password", "••••••••"], ["Phone Number", "phoneNumber", "+1 234 567 890"], ["Special Question", "specialQuestion", "Security question"], ["Special Answer", "specialAnswer", "Answer"], ["Assigned Person", "assignedPerson", "Name"], ["Assigned Device", "assignedDevice", "Device"], ["Assigned IP", "assignedIp", "IP"]] as [string, string, string][]).map(([label, key, ph]) => (
            <FormField key={key} label={label}>
              <input value={(form as Record<string, string | boolean>)[key] as string} onChange={e => setForm({ ...form, [key]: e.target.value })} className={inputClass()} placeholder={ph} />
            </FormField>
          ))}
